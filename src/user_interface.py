...
class UserInterface:
            ...
            if not cities[0]:
                ...
                while True:
                    ...
                    if not city_number.isdigit() or int(city_number) < 0 or int(city_number) > len(
                        ...
                        cities = self.api_operations.get_city(final_city)
                        #print(f'Поиск будет производиться по {cities[1]}.')
                        break
                break
    ...
    def get_new_vacancy(self):
        ...
        if self.load_vacancies_from_api() == False:
            return False
        self.data_set_number = -1

    def data_set_choise(self):
        ...
        for i in range(len(self.v_d_operations.data)):
            print(f"{i}: город {self.v_d_operations.data[i]['city']}, запрос: {self.v_d_operations.data[i]['key_words']}")
        print()
    ...
    def sort_and_show(self):
        ...
        print()
        if top == 0:
            upper_border = min(len(self.v_d_operations.data[self.data_set_number]['data']), 10)
    ...
    def del_set_of_data(self):
        ...
        num = self.choose_one({0: 'отмена', 1: 'удаление'})
        if num:
            self.v_d_operations.del_data(self.data_set_number)

    def choose_one(self, choose_dict):
        ...
        while True:
            ...
            num = input()
            if not num.isdigit() or int(num) < min(choose_dict) or int(num) > max(choose_dict):
                print('должен быть введён номер из предложенных')
                continue
            else:
                return int(num)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if num == 2:
            ...
            search = 1
            while user_session.get_new_vacancy() == False:
